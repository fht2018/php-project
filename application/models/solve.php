    ...
    if($token){
         ...
         if($sn){
              ...
              if($da->level>=5){//验证是否拥有权限
                           ...
                           $array = array('group' => $da->group, 'level <' => $da->level);
                          ...
                          $this->load->view('......',$data);
                          $arr = array('status' => 0, 'info' => '查看成员列表成功');
         ...
         if($sn){//2个if验证token是否存在，token是否有效
              ...
              $da1=$this->db->where('id',$id)->get('w2_group')->first_row();//被查看者的权限
                    if(($da->level>=5)&&($da->level>=$da1->level)){ //验证是否拥有权限
                                   ...
                                   $arr = array('status' => 0, 'info' => '获取成功','data'=> $data1[0]);
                                       echo json_encode($arr);exit();         
                        }else{
                            $arr = array('status' => -1, 'info' => '您的权限不足,无法查看');
                            echo json_encode($arr);exit();
                        }
         }else{
            $arr = array('status' => -1, 'info' => '无效，请重新登录');
         ...
         if($sn){//2个if验证token是否存在，token是否有效
              ...
              if($da->group==$da1->group){
                      if(($da->level>=5)&&($da->level>$da1->level)){ //验证是否拥有权限
                            ...
                            $ar = array('position'=>'tourists','level' => 0);
                            $this->db->where('id',$id);
                            $sql = $this->db->update('w2_group', $ar);

                            $arr = array('status' => 0, 'message' => '权限游客修改成功');
                            echo json_encode($arr);exit();
                            }else if($json->position == 'informal'){

                            $ar = array('position'=>'informal','level' => 1);
                            $this->db->where('id',$id);
                            $sql = $this->db->update('w2_group', $ar);

                            $arr = array('status' => 0, 'message' => '权限考核成员修改成功');
                            echo json_encode($arr);exit();
                            }else if($json->position == 'formal'){

                            $ar = array('position'=>'formal','level' => 3);
                            $this->db->where('id',$id);
                            $sql = $this->db->update('w2_group', $ar);

                            $arr = array('status' => 0, 'message' => '权限正式成员修改成功');
                            echo json_encode($arr);exit();
                            }else if($json->position == 'header'){

                            $ar = array('position'=>'header','level' =>5);
                            $this->db->where('id',$id);
                            $sql = $this->db->update('w2_group', $ar);

                            $arr = array('status' => 0, 'message' => '权限小组长修改成功');
                            echo json_encode($arr);exit();
                            }else if($json->position == 'leader'){

                            $ar = array('position'=>'leader','level' => 7);
                            $this->db->where('id',$id);
                            $sql = $this->db->update('w2_group', $ar);

                            $arr = array('status' => 0, 'message' => '权限负责人修改成功');
                            echo json_encode($arr);exit();
                            }
                        }else{
                            $arr = array('status' => -1, 'info' => '您的权限不足');
                            ...
                            $arr = array('status' => -1, 'info' => '无法修改其他用户组成员的权限');
                            echo json_encode($arr);exit();
                        }
         }else{
            $arr = array('status' => -1, 'info' => '无效，请重新登录');
